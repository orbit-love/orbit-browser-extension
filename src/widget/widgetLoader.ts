...
export default class WidgetLoader {
  ...
  async init() {
    ...
    for (const widgetZone of widgetZones) {
      ...
      const buttonElement = window.document.createElement(`obe-${this.platform}-button`);
      ...
      const additionalDataComponent = window.document.createElement(`obe-${this.platform}-additional-data`);
